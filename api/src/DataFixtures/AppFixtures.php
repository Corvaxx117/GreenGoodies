...
final class AppFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        // Tous les produits de démonstration sont rattachés au compte marchand pour alimenter les deux catalogues.
        foreach ($this->catalog() as $productData) {
            ...
            $products[$productData['slug']] = $product;
        }

        $this->seedOrders($manager, $merchant, $customer, $products);
    ...
    private function seedOrders(ObjectManager $manager, User $merchant, User $customer, array $products): void
    {
        ...
        foreach ([
            [
                'user' => $merchant,
                'items' => [
                    ['slug' => 'kit-hygiene-recyclable', 'quantity' => 1],
                    ['slug' => 'gourde-en-bois', 'quantity' => 2],
                ],
            ],
            [
                'user' => $merchant,
                'items' => [
                    ['slug' => 'bougie-lavande-patchouli', 'quantity' => 1],
                    ['slug' => 'savon-bio', 'quantity' => 3],
                ],
            ],
            [
                'user' => $merchant,
                'items' => [
                    ['slug' => 'disques-demaquillants-x3', 'quantity' => 2],
                ],
            ],
            [
                'user' => $merchant,
                'items' => [
                    ['slug' => 'kit-couvert-en-bois', 'quantity' => 1],
                    ['slug' => 'brosse-a-dent', 'quantity' => 2],
                ],
            ],
            [
                'user' => $merchant,
                'items' => [
                    ['slug' => 'shot-tropical', 'quantity' => 4],
                    ['slug' => 'savon-bio', 'quantity' => 1],
                ],
            ],
            [
                'user' => $customer,
                'items' => [
                    ['slug' => 'gourde-en-bois', 'quantity' => 1],
                    ['slug' => 'necessaire-deodorant-bio', 'quantity' => 1],
                ],
            ],
        ] as $orderData) {
            $order = new CustomerOrder($orderData['user']);

            foreach ($orderData['items'] as $itemData) {
                $order->addItem($products[$itemData['slug']], $itemData['quantity']);
            }
